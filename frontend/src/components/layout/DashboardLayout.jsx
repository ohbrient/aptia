...
import {
  ...
  Users, Users2, BarChart3, Map, Activity, Zap, LogOut, ChevronDown,
  Bell, Lock, AlertTriangle, X as XIcon, Wind, RefreshCw
} from 'lucide-react';
...
const NAV = {
  ...
  rrhh: [
    ...
    { section:'Organizacional', links:[
      { to:'/rrhh/clima',   icon:Wind,     label:'Clima Laboral' },
      { to:'/rrhh/eval360', icon:RefreshCw,label:'Evaluación 360°' },
    ]},
...
function Avatar({ nombre }) {
  ...
  return <div className="w-8 h-8 rounded-full bg-brand-600 text-white text-xs font-bold flex items-center justify-center flex-shrink-0">{initials}</div>;
}
...
function BannerLicencia({ rol }) {
  ...
  const { data: licencias=[] } = useQuery({
    ...
    enabled: rol==='rrhh', staleTime:1000*60*10,
  });
  ...
  if (vencida) return (
    <div className="bg-red-600 text-white px-6 py-3 flex items-center gap-3 text-sm">
      ...
      <div className="flex-1"><p className="font-bold">⚠️ Tu licencia ha vencido</p><p className="text-red-200 text-xs mt-0.5">El acceso está restringido. Contacta a tu proveedor.</p></div>
      <a href="/rrhh/licencias" className="bg-white text-red-600 font-bold text-xs px-4 py-1.5 rounded-lg flex-shrink-0">Renovar →</a>
    </div>
  );
  if (cerrado) return null;
  const dias = porVencer?Math.ceil((new Date(porVencer.fecha_vencimiento)-hoy)/(1000*60*60*24)):null;
  const pct2 = casillena?Math.round((casillena.candidatos_usados/casillena.candidatos_total)*100):null;
  if (!porVencer&&!casillena) return null;
  const msg = porVencer
    ?`Tu licencia vence en ${dias===0?'hoy':`${dias} día${dias!==1?'s':''}`}. Renueva para no perder acceso.`
    :`Has usado el ${pct2}% de tus candidatos. Amplía tu licencia.`;
  return (
    <div className="bg-amber-500 text-white px-6 py-2.5 flex items-center gap-3 text-sm">
      ...
      <a href="/rrhh/licencias" className="underline font-bold flex-shrink-0">Ver licencias →</a>
      <button onClick={()=>setCerrado(true)} className="p-0.5 hover:bg-amber-600 rounded ml-2 flex-shrink-0"><XIcon className="w-3.5 h-3.5"/></button>
    </div>
  );
}

function BannerDemo({ rol }) {
  if (rol!=='rrhh') return null;
  ...
  const dias = expira?Math.ceil((new Date(expira)-Date.now())/(1000*60*60*24)):7;
  if (dias<=0) return null;
  return (
    <div className="bg-violet-600 text-white px-6 py-2.5 flex items-center gap-3 text-sm">
      <Zap className="w-4 h-4 flex-shrink-0"/>
      <p className="flex-1 font-medium">Estás en modo <strong>DEMO</strong> — acceso completo por {dias} día{dias!==1?'s':''} más.</p>
      <a href="mailto:ventas@aptia.io" className="bg-white text-violet-600 font-bold text-xs px-3 py-1.5 rounded-lg flex-shrink-0">Contratar ahora →</a>
    </div>
  );
}
...
function DemoExpiredGuard({ rol, children }) {
  ...
  if (rol!=='rrhh'||!onboarding?.empresa?.es_demo) return children;
  const expira = onboarding?.empresa?.demo_expira_at;
  if (!expira||new Date(expira)>Date.now()) return children;
  return (
    <div className="flex flex-1 items-center justify-center bg-slate-50">
      <div className="text-center max-w-md px-8">
        <div className="w-20 h-20 bg-violet-100 rounded-3xl flex items-center justify-center mx-auto mb-6"><Zap className="w-10 h-10 text-violet-600"/></div>
        <h2 className="text-2xl font-bold text-slate-900 mb-3">Tu demo ha expirado</h2>
        <p className="text-slate-500 mb-8">El período de prueba de <strong>7 días</strong> ha finalizado.</p>
        <a href="mailto:ventas@aptia.io" className="btn-primary inline-flex items-center gap-2"><Zap className="w-4 h-4"/> Contratar Aptia</a>
      </div>
    </div>
  );
}
...
function LicenciaVencidaGuard({ rol, children }) {
  ...
  const vencida = licencias.find(l=>l.activa&&new Date(l.fecha_vencimiento)<new Date());
  if (!vencida) return children;
  return (
    <div className="flex flex-1 items-center justify-center bg-slate-50">
      <div className="text-center max-w-md px-8">
        <div className="w-20 h-20 bg-red-100 rounded-3xl flex items-center justify-center mx-auto mb-6"><AlertTriangle className="w-10 h-10 text-red-600"/></div>
        <h2 className="text-2xl font-bold text-slate-900 mb-3">Licencia vencida</h2>
        <p className="text-slate-500 mb-8">Tu licencia venció el <strong>{new Date(vencida.fecha_vencimiento).toLocaleDateString('es-DO',{year:'numeric',month:'long',day:'numeric'})}</strong>.</p>
        <a href="mailto:soporte@aptia.com" className="btn-primary inline-flex items-center gap-2">Contactar soporte →</a>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <header className="h-14 bg-white border-b border-slate-200 flex items-center px-6 gap-4 sticky top-0 z-30">
        ...
        <span className="text-sm text-slate-500 font-medium truncate max-w-[200px]">{user?.empresa_nombre||rolLabel[rol]}</span>
        <div className="flex-1"/>
        <button className="relative p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-50 rounded-lg transition-colors"><Bell className="w-4 h-4"/></button>
        <div className="relative" ref={menuRef}>
          <button onClick={()=>setUserMenu(v=>!v)} className="flex items-center gap-2.5 pl-3 pr-2 py-1.5 rounded-xl hover:bg-slate-50 transition-colors">
          ...
          {userMenu&&(
            <div className="absolute right-0 top-full mt-1 w-52 bg-white border border-slate-200 rounded-xl shadow-lg z-50 overflow-hidden">
              ...
              <button onClick={()=>{setUserMenu(false);setCambiarPwd(true);}} className="w-full flex items-center gap-2.5 px-4 py-3 text-sm text-slate-600 hover:bg-slate-50 transition-colors border-b border-slate-100">
                <Lock className="w-4 h-4"/> Cambiar contraseña
              </button>
              <button onClick={handleLogout} className="w-full flex items-center gap-2.5 px-4 py-3 text-sm text-red-600 hover:bg-red-50 transition-colors">
          ...
          <aside className="w-56 bg-white border-r border-slate-200 flex flex-col py-4 sticky top-14 h-[calc(100vh-56px)] overflow-y-auto flex-shrink-0">
            {(NAV[rol]||[]).map(section=>(
              <div key={section.section} className="mb-4 px-3">
                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 px-3 mb-1.5">{section.section}</p>
                {section.links.map(link=>(
                  <NavLink key={link.to} to={link.to} end={link.end}
                    className={({isActive})=>`flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm font-medium mb-0.5 transition-colors ${isActive?'bg-brand-50 text-brand-700':'text-slate-600 hover:text-slate-900 hover:bg-slate-50'}`}
                  >
                    <link.icon className="w-4 h-4 flex-shrink-0"/>{link.label}
                  </NavLink>
                ))}
              </div>
            ))}
            <div className="mt-auto px-3">
              <button onClick={handleLogout} className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm font-medium text-red-500 hover:bg-red-50 hover:text-red-700 transition-colors">
                <LogOut className="w-4 h-4"/> Cerrar sesión
              </button>
            </div>
          </aside>
          <main className="flex-1 bg-slate-50 min-h-[calc(100vh-56px)]"><Outlet/></main>
        </div>
