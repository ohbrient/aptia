...
function ModalEditarItem({ item, dimensiones, onClose, onSave }) {
  ...
  const mut = useMutation({
    mutationFn: d => api.put(`/rrhh/banco/items/${item.id}`, d),
    onSuccess: () => { onSave(); onClose(); },
...
export default function PruebaDetalle() {
  ...
  const { data: prueba } = useQuery({
    queryKey: ['rrhh-prueba-detalle', id],
    queryFn: ()=>api.get(`/rrhh/banco/pruebas`).then(r=>r.data.find(p=>p.id===id)),
  });

  const { data: dimensiones=[] } = useQuery({
    queryKey: ['rrhh-prueba-dims', id],
    queryFn: ()=>api.get(`/rrhh/banco/pruebas/${id}/dimensiones`).then(r=>r.data),
  });

  const { data: items=[] } = useQuery({
    queryKey: ['rrhh-prueba-items', id],
    queryFn: ()=>api.get(`/rrhh/banco/pruebas/${id}/items`).then(r=>r.data),
  });

  const eliminarItem = useMutation({
    mutationFn: itemId=>api.delete(`/rrhh/banco/items/${itemId}`),
    onSuccess: ()=>qc.invalidateQueries(['rrhh-prueba-items', id]),
  });

  const eliminarDim = useMutation({
    mutationFn: dimId=>api.delete(`/rrhh/banco/dimensiones/${dimId}`),
    onSuccess: ()=>qc.invalidateQueries(['rrhh-prueba-dims', id]),
  });

  const actualizarTiempo = useMutation({
    mutationFn: t => api.put(`/rrhh/banco/pruebas/${id}`, { tiempo_limite: t || null }),
    onSuccess: () => { invalidar(); setEditTiempo(false); },
  });

  const invalidar = () => {
    qc.invalidateQueries(['rrhh-prueba-items', id]);
    qc.invalidateQueries(['rrhh-prueba-dims', id]);
    qc.invalidateQueries(['rrhh-banco-pruebas']);
    qc.invalidateQueries(['rrhh-prueba-detalle', id]);;
  };
