...
import {
  ...
  ChevronDown, ChevronRight, ClipboardList, Layers, Pencil, Clock, Lock
} from 'lucide-react';
...
function ModalDimension({ pruebaId, onClose, onSave }) {
  ...
  const mut = useMutation({
    mutationFn: d=>api.post(`/rrhh/banco/pruebas/${pruebaId}/dimensiones`, d),
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
        ...
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div><label className="label">Nombre *</label><input type="text" value={form.nombre} onChange={e=>set('nombre',e.target.value)} placeholder="Ej: Apertura" className="input"/></div>
            <div><label className="label">Código *</label><input type="text" value={form.codigo} onChange={e=>set('codigo',e.target.value.toUpperCase())} placeholder="Ej: O" maxLength={10} className="input"/></div>
          </div>
          <div><label className="label">Descripción</label><input type="text" value={form.descripcion} onChange={e=>set('descripcion',e.target.value)} placeholder="Breve descripción" className="input"/></div>
          <div><label className="label">Orden</label><input type="number" min="0" value={form.orden} onChange={e=>set('orden',e.target.value)} className="input"/></div>
          {error && <p className="text-sm text-red-600 bg-red-50 px-4 py-3 rounded-lg">{error}</p>}
        </div>
        <div className="px-6 py-4 border-t border-slate-100 flex gap-3 justify-end">
          <button onClick={onClose} className="btn-secondary">Cancelar</button>
          <button onClick={()=>mut.mutate(form)} disabled={mut.isPending||!form.nombre||!form.codigo} className="btn-primary">{mut.isPending?'Guardando...':'Crear dimensión'}</button>
        </div>
      </div>
    </div>
  );
...
function ModalItem({ pruebaId, dimensiones, escala, totalItems, onClose, onSave }) {
  ...
  const opcionesDefault = escala === 'likert5' ? OPCIONES_LIKERT5
    ...
    : [];
  const mut = useMutation({
    mutationFn: d=>api.post(`/rrhh/banco/pruebas/${pruebaId}/items`, d),
    onSuccess: ()=>{ onSave(); onClose(); },
    onError: err=>setError(err.response?.data?.error||'Error'),
  });
  return (
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg">
        ...
        <div className="p-6 space-y-4">
          <div><label className="label">Texto del ítem *</label>
            <textarea value={texto} onChange={e=>setTexto(e.target.value)} rows={3} placeholder="Ej: Disfruto explorar nuevas ideas y conceptos." className="input resize-none"/>
          </div>
          ...
          {opcionesDefault.length > 0 && (
            <div className="bg-slate-50 rounded-xl p-3">
              ...
              <div className="flex flex-wrap gap-2">
                {opcionesDefault.map(o=><span key={o.orden} className="text-xs bg-white border border-slate-200 px-2 py-1 rounded-lg text-slate-600">{o.valor}. {o.texto}</span>)}
              </div>
            </div>
          )}
          {error && <p className="text-sm text-red-600 bg-red-50 px-4 py-3 rounded-lg">{error}</p>}
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex gap-3 justify-end">
          <button onClick={onClose} className="btn-secondary">Cancelar</button>
          <button onClick={()=>mut.mutate({ texto, dimension_id:dimId||null, orden:totalItems+1, opciones:opcionesDefault })} disabled={mut.isPending||!texto.trim()} className="btn-primary">
...
function ModalEditarItem({ item, dimensiones, esSistema, onClose, onSave }) {
  const [texto,    setTexto]    = useState(item.texto || '');
  const [dimId,    setDimId]    = useState(item.dimension_id || '');
  const [orden,    setOrden]    = useState(item.orden || 0);
  const [opciones, setOpciones] = useState(item.opciones?.map(o=>({...o})) || []);
  const [error,    setError]    = useState('');

  const endpoint = esSistema
    ? `/rrhh/pruebas/items/${item.id}`
    : `/rrhh/banco/items/${item.id}`;

  const mut = useMutation({
    mutationFn: d => api.put(endpoint, d),
    onSuccess: () => { onSave(); onClose(); },
    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="label">Dimensión</label>
              <select value={dimId} onChange={e=>setDimId(e.target.value)} className="input">
                <option value="">Sin dimensión</option>
          ...
          {opciones.length > 0 && (
            <div><label className="label">Opciones de respuesta</label>
              <div className="space-y-2">
                {opciones.map((op, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <span className="text-xs font-bold text-slate-400 w-5">{op.valor}.</span>
                    <input type="text" value={op.texto} onChange={e=>setOpcion(i,'texto',e.target.value)} className="input flex-1 py-1.5 text-sm"/>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex gap-3 justify-end">
          <button onClick={onClose} className="btn-secondary">Cancelar</button>
          <button onClick={()=>mut.mutate({ texto, dimension_id:dimId||null, orden, opciones })} disabled={mut.isPending||!texto.trim()} className="btn-primary">
...
function ModalExcel({ pruebaId, dimensiones, onClose, onSave }) {
  ...
  const handleFile = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        setPreview(items);
      } catch { setError('Error al leer el archivo. Verifica que sea .xlsx o .xls'); }
    };
  ...
  const handleUpload = async () => {
    setLoading(true);
    try {
      const res = await api.post(`/rrhh/banco/pruebas/${pruebaId}/items/bulk`, { items: preview });
        ...
        <div className="p-6">
          {resultado !== null ? (
            <div className="text-center py-8">
              <div className="w-16 h-16 bg-emerald-100 rounded-2xl flex items-center justify-center mx-auto mb-4"><ClipboardList className="w-8 h-8 text-emerald-600"/></div>
            ...
            <>
              <div className="bg-brand-50 border border-brand-200 rounded-xl p-4 mb-6">
                ...
                <button onClick={descargarPlantilla} className="btn-secondary mt-3 text-xs py-1.5 px-3"><Download className="w-3 h-3"/> Descargar plantilla</button>
              </div>
              <div onClick={()=>fileRef.current.click()} className="border-2 border-dashed border-slate-300 hover:border-brand-400 rounded-xl p-8 text-center cursor-pointer transition-colors mb-4">
              ...
              {preview.length > 0 && (
                <div>
                  <p className="text-sm font-bold text-slate-800 mb-3">Vista previa — {preview.length} ítem(s) detectados</p>
                  <div className="border border-slate-200 rounded-xl overflow-hidden mb-4 max-h-64 overflow-y-auto">
                    <table className="w-full text-xs">
                      <thead className="bg-slate-50 sticky top-0">
                        <tr>{['#','Texto','Dim.','Opciones'].map(h=><th key={h} className="text-left px-4 py-2 font-semibold text-slate-500">{h}</th>)}</tr>
                      </thead>
                      <tbody>
                        {preview.map((it,i)=>(
                          <tr key={i} className="border-t border-slate-100">
                            ...
                            <td className="px-4 py-2">{it.dimension_codigo?<span className="badge-blue">{it.dimension_codigo}</span>:<span className="text-slate-300">—</span>}</td>
                  ...
                  <div className="flex gap-3 justify-end">
                    ...
                    <button onClick={handleUpload} disabled={loading} className="btn-primary">{loading?'Cargando...':`Importar ${preview.length} ítems`}</button>
                  </div>
                </div>
              )}
            </>
...
export default function PruebaDetalle() {
  ...
  const esSistema = searchParams.get('source') === 'sistema';
  ...
  const idValido = !!id && id !== 'undefined';

  // ── Query: datos de la prueba ──────────────────────────────
  const { data: prueba } = useQuery({
    queryKey: ['rrhh-prueba-detalle', id, esSistema],
    enabled: idValido,
    queryFn: async () => {
      if (esSistema) {
        const r = await api.get('/rrhh/pruebas-disponibles');
        return r.data.find(p => String(p.id) === String(id)) ?? null;
      } else {
        const r = await api.get('/rrhh/banco/pruebas');
        return r.data.find(p => String(p.id) === String(id)) ?? null;
      }
    },
    select: d => d,
  ...
  const { data: dimensiones=[] } = useQuery({
    queryKey: ['rrhh-prueba-dims', id, esSistema],
    enabled: idValido,
    queryFn: () => esSistema
      ? api.get(`/rrhh/pruebas/${id}/dimensiones`).then(r => r.data)
      : api.get(`/rrhh/banco/pruebas/${id}/dimensiones`).then(r => r.data),
  });

  // ── Query: ítems ───────────────────────────────────────────
  const { data: items=[] } = useQuery({
    queryKey: ['rrhh-prueba-items', id, esSistema],
    enabled: idValido,
    queryFn: () => esSistema
      ? api.get(`/rrhh/pruebas/${id}/items`).then(r => r.data)
      : api.get(`/rrhh/banco/pruebas/${id}/items`).then(r => r.data),
  });

  const eliminarItem = useMutation({
    mutationFn: itemId => api.delete(`/rrhh/banco/items/${itemId}`),
    onSuccess: () => invalidar(),
  });

  const eliminarDim = useMutation({
    mutationFn: dimId => api.delete(`/rrhh/banco/dimensiones/${dimId}`),
    onSuccess: () => invalidar(),
  });

  const actualizarTiempo = useMutation({
    mutationFn: t => esSistema
      ? api.patch(`/rrhh/pruebas/${id}/tiempo`, { tiempo_limite: t ? parseInt(t) : null })
      : api.put(`/rrhh/banco/pruebas/${id}`, { tiempo_limite: t || null }),
    onSuccess: () => {
      invalidar();
      setEditTiempo(false);
      qc.invalidateQueries(['rrhh-pruebas-disponibles']);
    },
  });

  const invalidar = () => {
    qc.invalidateQueries(['rrhh-prueba-items', id, esSistema]);
    qc.invalidateQueries(['rrhh-prueba-dims', id, esSistema]);
    qc.invalidateQueries(['rrhh-banco-pruebas']);
    qc.invalidateQueries(['rrhh-prueba-detalle', id, esSistema]);
  };
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={()=>navigate('/rrhh/banco')} className="p-2 hover:bg-slate-100 rounded-lg transition-colors">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-0.5">
            ...
            {esSistema && (
              <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-slate-100 text-slate-500 flex items-center gap-1">
                <Lock className="w-3 h-3"/> Prueba del sistema
              </span>
            )}
          </div>
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {editTiempo ? (
              <div className="flex items-center gap-2">
                <input type="number" min="1" max="180" value={nuevoTiempo} onChange={e=>setNuevoTiempo(e.target.value)} placeholder="min" className="input py-1 px-2 text-xs w-20" autoFocus/>
                <span className="text-xs text-slate-400">min</span>
                <button onClick={()=>actualizarTiempo.mutate(nuevoTiempo)} disabled={actualizarTiempo.isPending} className="text-xs bg-brand-600 text-white px-2 py-1 rounded-lg font-semibold">
              ...
              <button onClick={()=>{ setNuevoTiempo(prueba?.tiempo_limite||''); setEditTiempo(true); }} className="flex items-center gap-1 text-xs text-slate-500 hover:text-brand-600 bg-slate-100 hover:bg-brand-50 px-2 py-1 rounded-lg transition-all">
                <Clock className="w-3 h-3"/>
                {prueba?.tiempo_limite ? `${prueba.tiempo_limite} min` : 'Sin límite'} · Editar tiempo
              </button>
            )}
          </div>
        </div>
      ...
      {esSistema && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 mb-6 flex items-center gap-3">
          ...
          <p className="text-sm text-blue-700">Puedes ver y editar las preguntas de esta prueba, y modificar el tiempo límite.</p>
        </div>
      )}
        ...
        <div>
          ...
          {dimensiones.length===0 ? (
            <div className="card p-12 text-center">
              ...
              {!esSistema && <button onClick={()=>setModalDim(true)} className="btn-primary mx-auto mt-4"><Plus className="w-4 h-4"/> Crear dimensión</button>}
            </div>
          ) : (
            <div className="card overflow-hidden">
              <table className="w-full text-sm">
                <thead><tr className="border-b border-slate-100 bg-slate-50">
                  {['Código','Nombre','Descripción','Ítems',...(!esSistema?['']:[''])].map((h,i)=>(
                    ...
                    <tr key={d.id} className="border-b border-slate-50 hover:bg-slate-50">
                      ...
                      {!esSistema && (
                        <td className="px-5 py-4">
                          <button onClick={()=>eliminarDim.mutate(d.id)} className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg"><Trash2 className="w-4 h-4"/></button>
                        </td>
                      )}
                    </tr>
                  ))}
          ...
          {items.length===0 ? (
            <div className="card p-12 text-center">
              ...
              <div className="flex gap-3 justify-center mt-4">
                {!esSistema && <button onClick={()=>setModalExcel(true)} className="btn-secondary"><Upload className="w-4 h-4"/> Cargar Excel</button>}
                          ...
                          <div className="px-14 pb-3">
                            <div className="flex flex-wrap gap-2">
                              {item.opciones.map(op=>(
                                <span key={op.id} className="text-xs bg-slate-100 text-slate-600 px-2 py-1 rounded-lg">{op.valor}. {op.texto}</span>
                              ))}
                            </div>
                          </div>
                        )}
      ...
      {editandoItem && <ModalEditarItem item={editandoItem} dimensiones={dimensiones} esSistema={esSistema} onClose={()=>setEditandoItem(null)} onSave={invalidar}/>}
