      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        {step===1&&(
          <div className="p-6">
            ...
            {clientes.length===0?(<div className="text-center py-8"><Building2 className="w-10 h-10 text-slate-200 mx-auto mb-2"/><p className="text-sm text-slate-400">No hay empresas cliente registradas.</p></div>):(
              <div className="space-y-2">{clientes.map(c=>(
      ...
      <tr className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
        ...
        <td className="px-5 py-4">
          <div className="flex gap-2 items-center">
            <button onClick={()=>onPerfil(proceso)} className="p-1.5 text-violet-500 hover:text-violet-700 hover:bg-violet-50 rounded-lg transition-colors" title="Definir perfil de puesto">
              <Target className="w-3.5 h-3.5"/>
            </button>
            <button onClick={()=>onComparar(proceso)} className="p-1.5 text-emerald-500 hover:text-emerald-700 hover:bg-emerald-50 rounded-lg transition-colors" title="Comparar candidatos">
      ...
      {open&&(
        <tr className="bg-slate-50 border-b border-slate-100">
          <td colSpan={7} className="px-8 py-4">
            {candidatos.length===0?(<p className="text-sm text-slate-400">No hay candidatos invitados aún.</p>):(
                ...
                {candidatos.map(c=>{
                  ...
                  return (
                    <div key={c.id} className="flex items-center gap-3 bg-white rounded-xl px-4 py-2.5 border border-slate-100">
                      <div className="w-7 h-7 rounded-full bg-brand-100 text-brand-700 text-xs font-bold flex items-center justify-center flex-shrink-0">{c.nombre[0]}{c.apellido?.[0]||''}</div>
                      <div className="flex-1"><p className="text-sm font-semibold text-slate-800">{c.nombre} {c.apellido}</p><p className="text-xs text-slate-400">{c.email}</p></div>
