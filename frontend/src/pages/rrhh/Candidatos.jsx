        ...
        <div className="p-6 space-y-5">
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="label">Nombre *</label><input type="text" value={form.nombre} onChange={e=>set('nombre',e.target.value)} className="input" placeholder="Juan"/></div>
              <div><label className="label">Apellido</label><input type="text" value={form.apellido} onChange={e=>set('apellido',e.target.value)} className="input" placeholder="Pérez"/></div>
            </div>
            <div className="mt-3"><label className="label">Email *</label><input type="email" value={form.email} onChange={e=>set('email',e.target.value)} className="input" placeholder="juan@email.com"/></div>
          </div>
          <div>
            <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Información personal</p>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="label">Teléfono</label><input type="text" value={form.telefono} onChange={e=>set('telefono',e.target.value)} className="input" placeholder="809-000-0000"/></div>
              <div><label className="label">Cédula / Documento</label><input type="text" value={form.cedula} onChange={e=>set('cedula',e.target.value)} className="input" placeholder="000-0000000-0"/></div>
              <div><label className="label">Fecha de nacimiento</label><input type="date" value={form.fecha_nacimiento} onChange={e=>set('fecha_nacimiento',e.target.value)} className="input"/></div>
              <div>
              ...
              <div><label className="label">Ciudad</label><input type="text" value={form.ciudad} onChange={e=>set('ciudad',e.target.value)} className="input" placeholder="Santo Domingo"/></div>
              <div><label className="label">País</label><input type="text" value={form.pais} onChange={e=>set('pais',e.target.value)} className="input" placeholder="República Dominicana"/></div>
            </div>
          </div>
          <div>
            <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Otros datos</p>
            <div><label className="label">LinkedIn</label><input type="url" value={form.linkedin} onChange={e=>set('linkedin',e.target.value)} className="input" placeholder="https://linkedin.com/in/..."/></div>
            <div className="mt-3"><label className="label">Notas internas</label><textarea value={form.notas} onChange={e=>set('notas',e.target.value)} className="input resize-none" rows={3} placeholder="Observaciones sobre el candidato..."/></div>
          </div>
          {error && <p className="text-sm text-red-600 bg-red-50 px-4 py-3 rounded-lg">{error}</p>}
          <div className="flex gap-3 justify-end pt-1">
            <button onClick={onClose} className="btn-secondary">Cancelar</button>
            <button onClick={() => mut.mutate()} disabled={mut.isPending || !form.nombre || !form.email} className="btn-primary">
...
function ModalCandidato({ candidato: c, onClose, onEditar, onEtapaCambio }) {
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 sticky top-0 bg-white z-10">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => { onClose(); onEditar(c); }} className="btn-secondary py-2 px-4 text-xs flex items-center gap-1">
              <Pencil className="w-3.5 h-3.5"/> Editar
            </button>
            {(c.estado === 'pendiente' || c.estado === 'en_progreso') && (
              <button onClick={async () => {
                try { await api.post(`/rrhh/candidatos/${c.id}/reenviar`); alert('Link reenviado a ' + c.email); }
            ...
            {c.estado === 'completado' && (
              <button onClick={() => abrirReporte(c.id)} className="btn-primary py-2 px-4 text-xs flex items-center gap-1">
        ...
        <div className="p-6">
          ...
          {c.estado === 'completado' && c.match_score != null && (
            <div className="mb-5"><MatchBadge score={c.match_score} nivel={c.match_nivel} size="lg"/></div>
          )}

          {/* Etapa de reclutamiento */}
          <div className="flex items-center gap-3 mb-5 bg-slate-50 rounded-xl px-4 py-3">
            <p className="text-xs font-bold uppercase tracking-wider text-slate-400 flex-shrink-0">Etapa</p>
            <EtapaSelector
              candidatoId={c.id}
              etapaActual={c.etapa_reclutamiento}
              onCambio={onEtapaCambio}
            />
          </div>
          ...
          <div className="bg-slate-50 rounded-xl p-4 mb-6 grid grid-cols-2 gap-4 text-sm">
            ...
            {c.fecha_nacimiento && <div><p className="text-xs text-slate-400 mb-0.5">Fecha nacimiento</p><p className="font-medium text-slate-800">{new Date(c.fecha_nacimiento).toLocaleDateString('es-DO',{year:'numeric',month:'long',day:'numeric'})}</p></div>}
            {c.genero && <div><p className="text-xs text-slate-400 mb-0.5">Género</p><p className="font-medium text-slate-800 capitalize">{c.genero.replace('_',' ')}</p></div>}
            {(c.ciudad || c.pais) && <div><p className="text-xs text-slate-400 mb-0.5">Ubicación</p><p className="font-medium text-slate-800">{[c.ciudad, c.pais].filter(Boolean).join(', ')}</p></div>}
            {c.linkedin && <div><p className="text-xs text-slate-400 mb-0.5">LinkedIn</p><a href={c.linkedin} target="_blank" rel="noreferrer" className="text-brand-600 font-medium text-xs hover:underline">Ver perfil</a></div>}
          ...
          {exito ? (
            <div>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div><p className="text-sm font-bold text-slate-900">¡Candidato creado!</p><p className="text-xs text-slate-400">Email enviado · Link disponible</p></div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="label">Nombre *</label><input type="text" value={form.nombre} onChange={e=>set('nombre',e.target.value)} placeholder="Juan" className="input"/></div>
                <div><label className="label">Apellido</label><input type="text" value={form.apellido} onChange={e=>set('apellido',e.target.value)} placeholder="Pérez" className="input"/></div>
              </div>
              <div><label className="label">Email *</label><input type="email" value={form.email} onChange={e=>set('email',e.target.value)} placeholder="juan@email.com" className="input"/></div>
              {error && <p className="text-sm text-red-600 bg-red-50 px-4 py-3 rounded-lg">{error}</p>}
              <div className="flex gap-3 justify-end pt-2">
                <button onClick={onClose} className="btn-secondary">Cancelar</button>
                <button onClick={() => mut.mutate()} disabled={mut.isPending || !form.nombre || !form.email || !form.proceso_id} className="btn-primary">
...
// ── Página principal ──────────────────────────────────────
export default function Candidatos({ endpoint='/rrhh/candidatos-todos', showEmpresa=false }) {
  ...
  const queryKey = ['candidatos-todos', endpoint];
  ...
  // Actualizar etapa en cache local sin refetch
  const handleEtapaCambio = (candidatoId, nuevaEtapa) => {
    qc.setQueryData(queryKey, old =>
      (old||[]).map(c => c.id === candidatoId ? { ...c, etapa_reclutamiento: nuevaEtapa } : c)
    );
    if (detalle?.id === candidatoId) {
      setDetalle(d => d ? { ...d, etapa_reclutamiento: nuevaEtapa } : d);
    }
  };
  ...
  const filtrados = candidatos.filter(c => {
    ...
    const etapa = !filtroEtapa || (filtroEtapa === 'sin_etapa' ? !c.etapa_reclutamiento : c.etapa_reclutamiento === filtroEtapa);
    const txt   = !q ||
      c.nombre?.toLowerCase().includes(q) ||
      c.apellido?.toLowerCase().includes(q) ||
      c.email?.toLowerCase().includes(q) ||
      c.proceso_nombre?.toLowerCase().includes(q);
    return est && etapa && txt;
  });
    ...
    <div className="p-8">
      ...
      <div className="flex gap-3 mb-5 flex-wrap items-center">
        ...
        <div className="flex gap-1">
          {[['','Todos'],['completado','Completados'],['en_progreso','En progreso'],['pendiente','Pendientes'],['expirado','Expirados']].map(([k,l])=>(
            <button key={k} onClick={()=>setFiltroEst(k)} className={`text-xs px-3 py-1.5 rounded-lg font-medium transition-all ${filtroEst===k?'bg-brand-600 text-white':'bg-white border border-slate-200 text-slate-600 hover:border-slate-300'}`}>{l}</button>
          ))}
        </div>
        {/* Filtro etapa */}
        <div className="flex gap-1">
          {[['','Todas las etapas'],['entrevistado','Entrevistados'],['contratado','Contratados'],['no_contratado','No contratados'],['sin_etapa','Sin etapa']].map(([k,l])=>(
            <button key={k} onClick={()=>setFiltroEtapa(k)} className={`text-xs px-3 py-1.5 rounded-lg font-medium transition-all ${filtroEtapa===k?'bg-slate-700 text-white':'bg-white border border-slate-200 text-slate-600 hover:border-slate-300'}`}>{l}</button>
          ))}
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                {['Candidato','Proceso','Estado','Etapa','Match','Invitado','Completado',''].map(h=>(
                  ...
                  <tr key={c.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors cursor-pointer" onClick={()=>setDetalle(c)}>
                    ...
                    <td className="px-5 py-4" onClick={e=>e.stopPropagation()}>
                      <div className="flex items-center gap-1">
                        {(c.estado === 'pendiente' || c.estado === 'en_progreso') && (
                          <button onClick={(e) => reenviarLink(c, e)} disabled={reenviando === c.id} title="Reenviar link"
                            className={`p-1.5 rounded-lg transition-all flex items-center gap-1 text-xs font-semibold ${reenvioOk===c.id?'bg-emerald-100 text-emerald-700':'bg-amber-50 text-amber-600 hover:bg-amber-100'}`}>
                            {reenviando===c.id?<RefreshCw className="w-3.5 h-3.5 animate-spin"/>:reenvioOk===c.id?'✓ Enviado':<><Send className="w-3.5 h-3.5"/> Reenviar</>}
                          </button>
                        )}
                        {c.estado === 'completado' && (
                          <button onClick={() => abrirReporte(c.id)} title="Ver reporte PDF" className="p-1.5 text-brand-500 hover:text-brand-700 hover:bg-brand-50 rounded-lg transition-colors">
                            <FileText className="w-3.5 h-3.5"/>
                          </button>
                        )}
                        <button onClick={() => setEditar(c)} title="Editar candidato" className="p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-colors">
                          <Pencil className="w-3.5 h-3.5"/>
                        </button>
                        <button onClick={() => setDetalle(c)} title="Ver detalle" className="p-1.5 text-brand-500 hover:text-brand-700 hover:bg-brand-50 rounded-lg transition-colors flex items-center gap-1">
      ...
      {detalle && (
        <ModalCandidato
          ...
          onEtapaCambio={(nuevaEtapa) => handleEtapaCambio(detalle.id, nuevaEtapa)}
        />
      )}
      {editar && (
